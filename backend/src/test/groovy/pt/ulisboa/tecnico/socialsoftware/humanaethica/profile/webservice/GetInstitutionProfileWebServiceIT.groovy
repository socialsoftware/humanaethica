...
class GetInstitutionProfileWebServiceIT extends SpockTest {
    ...
    def setup() {
        ...
        and: "a created institution profile"
        institutionProfileService.createInstitutionProfile(institution.id, institutionProfileDto)
    }

    def 'member of an institution gets the profile of the same institution'() {
        given:
        demoMemberLogin()

        when:
        def response = webClient.get()
                .uri('/profile/institution/' + institution.id)
    ...
    def "a volunteer gets an institution's profile"() {
        given:
        demoVolunteerLogin()
    ...
    def "a non authenticated user gets an institution's profile"() {
        when:
        def response = webClient.get()
                .uri('/profile/institution/' + institution.id)
                .headers(httpHeaders -> httpHeaders.putAll(headers))
    ...
    def 'a non authenticated user gets non-existent profile'() {
        given: "a different institution without a profile"
        def institutionWithoutProfile = createInstitution(INSTITUTION_2_NAME, INSTITUTION_2_EMAIL, INSTITUTION_2_NIF)

        when:
        def response = webClient.get()
                .uri('/profile/institution/' + institutionWithoutProfile.id)
                .headers(httpHeaders -> httpHeaders.putAll(headers))
                .retrieve()
                .bodyToMono(InstitutionProfileDto.class)
                .block()
