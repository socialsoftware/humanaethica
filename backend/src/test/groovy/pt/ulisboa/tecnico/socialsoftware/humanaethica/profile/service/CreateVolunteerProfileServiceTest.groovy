...
class CreateVolunteerProfileServiceTest extends SpockTest {
    ...
    @Unroll
    def 'invalid arguments: volunteerId=#volunteerId | dtoValue=#dtoValue'() {
        given:
        def volunteerProfileDto = new VolunteerProfileDto()

        when:
        volunteerProfileService.createVolunteerProfile(getProfileObjectId(volunteerId, volunteer), getExistingOrNullDto(dtoValue, volunteerProfileDto))
        ...
        where:
        volunteerId |  dtoValue || errorMessage
        null        |  EXIST     || ErrorMessage.USER_NOT_FOUND
        NO_EXIST    |  EXIST     || ErrorMessage.USER_NOT_FOUND
        EXIST       |  null                    || ErrorMessage.VOLUNTEER_PROFILE_SHORT_BIO_TOO_SHORT
    }
